...
weather_dyf = glue_context.create_dynamic_frame.from_catalog(database=db_name, table_name=table_name)

# convert to a spark dataframe
weather_df = weather_dyf.toDF()
# convert the temperature column from celcius to fahrenheit
weather_df = weather_df.withColumn("temp_farenheit", convert_celcius_to_fahrenheit(weather_df.temp))

# convert the city column to lower-case
weather_df = weather_df.withColumn("city", lower(weather_df.city))

# convert the state column to lower-case
weather_df = weather_df.withColumn("state", lower(weather_df.state))
# concatenate the city and state to make ts-id
weather_df = weather_df.withColumn("ts_id", concat("city", lit("-"), "state"))

most_recent_load_dt = weather_df.agg(max(date_format("time", "yyyy-MM-dd")).alias("max_date")).collect()[0]["max_date"]

# convert back to a DynamicFrame
weather_tmp = DynamicFrame.fromDF(weather_df, glue_context, "flat")
# Rename, cast, and nest with apply_mapping
# old name, old type -> new name, new type
weather_tmp = weather_tmp.apply_mapping(
    [
        ("ts_id", "string", "ts_id", "string"),
        ("time", "string", "time", "string"),
        ("temp_farenheit", "float", "temp_farenheit", "float"),
    ]
)
